  ...
  return `
    <div class="chat-header">
      ...
      <div class="chat-header-meta">
        ...
        <button class="btn-label-toggle btn-wa-external" id="btn-wa-external" title="Responder por WhatsApp (abre wa.me)">
...
function bindChatConversation() {
  ...
  const waExtBtn = document.getElementById('btn-wa-external');
  if (waExtBtn) waExtBtn.onclick = async () => {
    const phone = chatState.selectedPhone;
    if (!phone) return;
    // Buscar último inbound para copiar como contexto.
    const lastInbound = [...chatState.messages].reverse().find(m => m.direction === 'inbound' && m.body && m.msg_type !== 'reaction');
    const contextText = lastInbound ? `Contexto del cliente: "${lastInbound.body.slice(0, 200)}"` : '';
    if (contextText && navigator.clipboard) {
      try { await navigator.clipboard.writeText(contextText); toast('Contexto copiado al portapapeles'); }
      catch (_) { /* noop */ }
    }
    // Abrir wa.me — funciona en celular (abre WA Business app) y desktop (abre WA Web/desktop).
