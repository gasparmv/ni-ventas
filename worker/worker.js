...
<body>
<div class="wrap">
  <div class="topbar"><a class="tb-btn" href="https://drive.google.com/drive/folders/1A_l-10UoaDZh6RrT3ww1m5o3e3zG_hZy" target="_blank" rel="noopener">&#127912; Descargar Illustrator</a><button class="tb-btn" id="b_guia">&#128214; Guia</button></div>
...
(function(){
  ...
  function guiaHtml(){
    ...
    h+='<p>Vendes <b>carteles de neon LED personalizados</b>: un objeto fisico hecho a medida. No es neon de gas: es <b>tira LED tipo neon</b> montada sobre una base de <b>acrilico</b> cortada con la forma del dise&ntilde;o (texto, logo o figura). Cada uno se fabrica desde cero segun el dise&ntilde;o y las medidas.</p>';
    h+='<p>Es <b>neon LED de primera generacion, doble pista</b>, de la mejor calidad, <b>importado por nosotros mismos</b>, con puntos de corte cada <b>2.5 cm</b>. El <b>cableado va disimulado por los laterales</b>, queda prolijo.</p>';
    ...
    h+='<ul>';
    h+='<li><b>Exterior / sol y lluvia:</b> los sellamos con <b>silicona fria (tipo Fastix)</b> para impermeabilizarlos. Si esta bajo alero o techo, mejor.</li>';
    h+='<li><b>Consumo y enchufe:</b> es LED, bajo consumo y larga vida (<b>mas de 5 a&ntilde;os encendidos</b>). La tira trabaja a <b>12V</b> y viene con su <b>fuente/transformador</b>, que se enchufa a 220V como cualquier electrodomestico.</li>';
    h+='<li><b>Instalacion:</b> viene listo para colgar y usar.</li>';
    h+='<li><b>Garantia:</b> 1 a&ntilde;o por fallas propias del producto (no cubre golpes ni da&ntilde;o externo).</li>';
    h+='</ul>';
    h+='<h2>3. Controladores (extras opcionales)</h2>';
    h+='<p>Le dan mas efectos y un encendido/apagado comodo. <b>No son RGB</b> (no cambian de color): regulan intensidad, efectos e intermitencia. Gran producto para sumarle valor al cartel y sumar ganancia facil con una <b>venta cruzada</b>.</p>';
    h+='<ul>';
    ...
    h+='<ul><li>Foto o imagen de referencia del dise&ntilde;o.</li><li>Medidas aprox (ancho y alto en cm).</li><li>Metros de neon y tramos (el video te ense&ntilde;a a sacarlos).</li><li>Interior o exterior.</li><li>Entrega: envio incluido o retiro (mas barato).</li></ul>';
    h+='<p><b>Que son los "tramos":</b> son la cantidad de cortes o trazos sueltos que tienen las lineas de neon del dise&ntilde;o. Cada letra suele ser un tramo (la "a" son dos), un circulo es un solo tramo. Es la cantidad de cortes que hay que hacer fisicamente en el neon, y cada corte es una soldadura mas &mdash; o sea mas mano de obra. Por eso a mas tramos, mas complejo y mas caro. Contalos bien.</p>';
    ...
    h+='<h3>Trucos al pasar el precio</h3>';
    h+='<ul><li><b>Numeros no redondos:</b> pasa $497.200, no $500.000. Un numero exacto se percibe "calculado y justo"; uno redondo, "inventado a ojo".</li><li><b>Anclaje:</b> mostra primero el precio del cartel grande y despues ofrece uno mas chico &mdash; el chico parece una ganga.</li></ul>';
    ...
    h+='<li><b>Cerra con se&ntilde;a</b> (ej. 50%) para arrancar la produccion.</li>';
    h+='</ul>';
    h+='<h3>El audio vende mas que el texto</h3>';
    h+='<p>Es el recurso mas potente y el menos usado. Un audio corto apenas entra la consulta &mdash; "hola, como estas? recien llego, en un rato te paso el presupuesto" &mdash; genera confianza. Y <b>presentate con tu nombre</b>, no como marca: "hola, soy Marcos de [tu local]" mejor que "hola, somos [marca]".</p>';
    h+='<h3>Seguimiento: el que no sigue, pierde</h3>';
    h+='<p>Se cierra <b>~1 de cada 10 presupuestos</b>, muchas veces despues de varios contactos. No tengas verg&uuml;enza de insistir:</p>';
    h+='<ul><li><b>Dia 1:</b> reenganche suave &mdash; "seguis pensando lo del cartel? cualquier cosa que necesites para decidir, avisame".</li><li><b>Dia 3:</b> sumale valor o urgencia &mdash; "si lo encargas esta semana lo sumo a la tanda", o un beneficio (dimmer/envio).</li><li><b>Dia 7:</b> ultimo empujon &mdash; "cierro el presupuesto, lo dejamos para mas adelante?".</li></ul>';
    h+='<p>El que dice "mas adelante" no es un descarte: guardalo y reescribile en ~1 mes.</p>';
    h+='<h3>Donde estan las ventas</h3>';
    h+='<p>El <b>local que esta por abrir o remodelar</b> es oro (cafeterias, hamburgueserias, etc.). El cartel es de lo ultimo que compran en el montaje, asi que meteles el lead time: "tardamos 15-20 dias, no me lo pidas 3 dias antes de abrir".</p>';
    ...
    h+='<ul><li><b>Produccion:</b> ~15 a 20 dias (puede estirarse en epoca de mucha demanda &mdash; no sobre-prometas).</li><li><b>Entrega:</b> en el cotizador elegis <b>envio incluido</b> (el costo del envio ya va dentro del precio) o <b>retiro en taller</b> (mas barato, se descuenta el envio). Lo despachamos directo a vos o directo a tu cliente.</li></ul>';
    h+='<h2>8. Preguntas frecuentes del cliente</h2>';
    var faq=[['Cuanto sale?','Depende del tama&ntilde;o y la complejidad. Pasame foto + medidas y cotizas al toque.'],['Sirve para exterior?','Si, va sellado. Evita el sol directo permanente y, si hay alero, mejor.'],['Cuanto tarda?','La produccion ~15-20 dias.'],['Puedo ver como queda antes de pagar?','Si, con una imagen de referencia o un ejemplo similar.'],['Hacen mi logo?','Si, se vectoriza y se adapta al neon. Pasa la imagen en la mejor calidad posible.'],['Consume mucho? Como se enchufa?','Es LED, bajo consumo. Viene con fuente y va a 220V.'],['Como se instala?','Viene listo para colgar y usar. Lo entregamos con agujeros: podes colgarlo con tanzas, amurarlo con unos tornillos o pegarlo con cinta bifaz VHB (tipo 3M). Despues solo acomodas el cable. Es SUPER sencillo.'],['Tiene garantia?','1 a&ntilde;o por fallas de fabrica (no cubre golpes ni da&ntilde;os externos ni en la logistica).'],['Se puede regular / manejar del celu?','Si, con un controlador (Slim / Remoto / App).'],['Lo necesito para una fecha','Se sincero con el tiempo: la produccion es de 15-20 dias. Si la fecha es real, empuja la se&ntilde;a ya.'],['Quiero algo mas chico o mas barato','Preguntale que presupuesto tiene en mente y dise&ntilde;a acorde. Ofrece reducir la medida o simplificar el dise&ntilde;o antes de perder la venta.'],['Tienen cuotas?','Para carteles va con se&ntilde;a (ej. 50%) para arrancar y el saldo al entregar. Tambien podes ofrecerles pagar con Mercado Pago por transferencia eligiendo una tarjeta de credito: a vos te llega en el momento y ellos lo pagan en cuotas (con el recargo que aplique la tarjeta que elijan).']];
    for(var f=0;f<faq.length;f++){ h+='<div class="qa"><b>'+faq[f][0]+'</b><p style="margin-top:2px">'+faq[f][1]+'</p></div>'; }
    h+='<h2>9. Que SI y que NO</h2>';
    h+='<p><b>SI:</b> responder rapido, mostrar como queda, ofrecer el controlador, seguir a los que no contestan, cerrar con se&ntilde;a.</p>';
    h+='<p><b>NO:</b> cotizar a ojo, prometer fechas imposibles, saturar con 20 medidas, cerrar sin se&ntilde;a.</p>';
    ...
    h+='<p><b>Regla de oro:</b> manda el presupuesto <b>siempre con un render</b> del cartel (la imagen de como queda). Acelera muchisimo la decision de compra.</p>';
...
DATOS QUE NO IMPROVISÁS — el FAQ del playbook está pensado para la etapa de venta (la maneja Joaco), así que hay datos que NO tenés que afirmar con seguridad en el relevamiento aunque el playbook los mencione. NUNCA inventes ni confirmes:
- COLORES: no tenés una lista de colores ni sabés si un color/estilo puntual (rosa, celeste, verde agua, degradé, multicolor) se puede hacer. No enumeres colores ni confirmes/niegues uno; decí que el color se define junto con el diseño.
- MEDIOS DE PAGO: para carteles lo único cierto es la seña del 50% por transferencia y el saldo al entregar. NO afirmes tarjeta, cuotas ni Mercado Pago (los medios de pago que figuran en la parte de CURSOS del playbook NO aplican a carteles).
- TIEMPOS: solo el tiempo general de producción (~15-20 días). NUNCA prometas una fecha exacta de calendario ni que "llega para tal día".
- OTROS PRODUCTOS: solo sabés con certeza que hacemos carteles de neón LED personalizados. Si preguntan por otra cartelería (chapa, luminosos comunes, gigantografías, tótems, corpóreas sin neón, etc.), no afirmes ni niegues que lo hacemos: decí que el equipo lo confirma.
- MEDIDAS — REGLA CRÍTICA, PROHIBIDO ROMPERLA: tu ÚNICA tarea con la medida es RELEVAR la que el cliente YA tiene en mente. TERMINANTEMENTE PROHIBIDO ofrecer, sugerir, listar, tirar de ejemplo o recomendar medidas — aunque el cliente no tenga una definida, aunque te pida que le tires opciones, y AUNQUE EL PLAYBOOK DIGA "ofrecele las medidas comunes" (ese consejo es para el humano; VOS no ofrecés medidas nunca). NUNCA le des medidas de ejemplo ni "las más comunes de tu rubro" (nada de "2 metros x 1 metro", "1.50 x 80", "1 metro x 60", ni NINGÚN número de tamaño puesto por vos), NUNCA armes una lista de opciones de tamaño, NUNCA opines que una medida "queda imponente / es ideal para exterior / se ve mejor / bien visible". Tampoco tenés criterio técnico: NUNCA afirmes una medida mínima ("tendrías que tener al menos X"), ni digas que un diseño entra / no entra / no se puede en tal medida. Qué hacés en cada caso: (a) si el cliente te da una medida (aunque sea aproximada) → la tomás y listo; (b) si NO tiene medida definida, o te PIDE que le recomiendes un tamaño, o pregunta cuál le conviene → NO le des ningún número: decile que igual lo cotizamos con una medida de referencia y que después se ajusta sin drama (ej: "no te compliques con la medida exacta ahora, pasame la que tengas en mente aunque sea muy aproximada y lo cotizamos igual, después la afinamos tranqui"); (c) si pregunta si su diseño entra / se puede hacer en tal medida → decile que eso lo define el equipo al armar el presupuesto.

PRESUPUESTO DEL CLIENTE — NUNCA le preguntes al cliente qué presupuesto tiene en mente ni cuánto quiere o puede gastar. En la pre cotización tu tarea es SOLO relevar los datos (foto, medidas, interior/exterior), NO negociar el precio ni sondear cuánto está dispuesto a pagar: preguntarlo tan temprano es prematuro y espanta. SOLO podés tocar el tema del presupuesto en DOS casos: (a) si ya es una negociación avanzada en la que el equipo YA le pasó un presupuesto, o (b) si el cliente vino pidiendo explícitamente algo BARATO / económico / lo más accesible. Fuera de esos dos casos, ni lo menciones. Y si el cliente no sabe qué medida quiere, NO le ofrezcas medidas para que elija ni "opciones en distintos precios" (ver la REGLA CRÍTICA de MEDIDAS de arriba): lo cotizamos con una medida aproximada y se ajusta después, sin preguntarle cuánto quiere gastar.

DISEÑO Y TIPOGRAFÍAS — las tipografías son SOLO para carteles hechos con LETRAS (un texto: el nombre del local, una palabra o una frase). Ofrecé y mandá las tipografías ÚNICAMENTE cuando se cumplen las DOS condiciones a la vez: (a) el cliente quiere un cartel de LETRAS/texto — NO un logo, dibujo, isotipo, personaje ni una imagen; y (b) el cliente NO tiene definida la tipografía o el estilo de letra (no sabe qué letra usar, o pide que le muestres opciones). En ese caso NO le exijas la "foto del diseño" (no la tiene): poné enviar_tipografias=true y avisale en texto natural que se las pasás para que elija (ej: "dale, te paso las tipografias que trabajamos y elegí la que más te guste"). El sistema adjunta las imágenes automáticamente. Después seguí pidiendo lo que falte (sobre todo las medidas). Mandá las tipografías UNA sola vez: si en la charla ya se las pasaste, NO pongas enviar_tipografias de nuevo.
NO mandes tipografías si el cliente quiere un LOGO / dibujo / imagen / isotipo (ahí lo que necesitás es la imagen del logo, NO una tipografía), o si el cliente YA te dijo o mostró qué tipografía o estilo de letra quiere. Si no te queda claro si es un cartel de letras o un logo, preguntáselo ANTES de mandar nada.
...
- pide VER TRABAJOS que hicimos, ejemplos, fotos de referencia, un catálogo, o "algún trabajo hecho / precio de referencia para tener idea": NO tenés fotos de trabajos anteriores ni un catálogo para mandar (lo ÚNICO que podés adjuntar son las tipografías, y solo para letras — ver arriba). NUNCA se lo prometas ni le digas "te paso unas fotos / algunos trabajos" (no lo podés cumplir) → frenar=true, motivo_freno="pide ver trabajos/ejemplos", que se lo mande Joaco a mano,
- pide que le MANDEMOS o HAGAMOS el RENDER / diseño / boceto / mockup / "una imagen de cómo quedaría" ANTES de avanzar ("me pasás un render?", "hacémelo y lo vemos", "diseñame algo primero"): VOS no hacés ni mandás el render (lo arma una persona junto con el presupuesto). NUNCA prometas un render/diseño/boceto, aunque el FAQ del playbook diga "te mando un render" (esas respuestas son para la etapa que maneja Joaco) → frenar=true, motivo_freno="pide render/diseño antes",
- pide o propone una LLAMADA / videollamada / VISITA, que lo llamen, o coordinar día y horario: VOS no agendás ni coordinás llamadas ni visitas ni podés prometer que alguien lo va a llamar. Si SOLO pregunta dónde estamos, decí "el barrio de Colegiales, CABA"; pero NUNCA acuerdes una llamada, visita, día u horario aunque el FAQ lo mencione → frenar=true, motivo_freno="pide llamada/visita",
- pide un LINK, WEB, catálogo online, lista de precios, PDF o cualquier archivo para ver/descargar: no podés mandar links ni archivos → frenar=true, motivo_freno="pide link/catálogo",
- REGATEA o pide descuento / promo / oferta / 2x1 / "precio mínimo" / "algo más barato" / precio por cantidad: vos no cotizás ni negociás ni autorizás descuentos, y no sabés qué promo está vigente → frenar=true, motivo_freno="regateo/descuento",
- YA es CLIENTE / post-venta: pregunta por el estado o envío de SU pedido, dice que su cartel llegó fallado / no prende, pide reponer/reparar, o quiere "otro igual al que ya me hicieron" → no le pidas foto/medidas como si fuera un lead nuevo, frenar=true, motivo_freno="post-venta / ya es cliente",
- quiere MODIFICAR o CANCELAR un pedido en curso (cambiar color/medida/texto, agregar algo, cancelar, cambiar la dirección de envío) → frenar=true, motivo_freno="modificar/cancelar pedido",
- está CONFUNDIDO o no reconoce el contacto ("de qué me hablás?", "quién sos?", "yo no pedí nada", "cómo tenés mi número?"): no sigas pidiendo datos ni inventes una explicación → frenar=true, motivo_freno="cliente confundido",
- te llega un AUDIO que ves como "[audio] (sin transcripción)" (no sabés qué dijo), o un mensaje largo con varias preguntas donde al menos una no la podés responder con el FAQ → no adivines ni contestes a medias inventando el resto, frenar=true, motivo_freno="audio sin transcribir / preguntas fuera del FAQ",
...
REGLA DE ORO — NO respondas por responder. Si NO sabés qué contestar con certeza, o el cliente pide algo que NO podés darle ni hacer (mandar fotos de ejemplos/catálogo, un dato que no tenés, algo fuera del relevamiento simple), NO improvises NI prometas algo que no vas a cumplir: poné frenar=true (que lo maneje Joaco). Prometer y no cumplir es MUCHO peor que frenar y escalar a una persona.
...
async function processPrecotizPilot(env) {
  ...
  for (const c of cands) {
    ...
    // Igual para leads del LANZAMIENTO (evento): quedan 'oculto' (no 'cursos') hasta que
    // respondan, así que el inbox no los excluye → los excluimos por la tabla.
    try { const lz = await env.DB.prepare("SELECT 1 AS x FROM lanzamiento_landing WHERE phone = ? LIMIT 1").bind(phone).first(); if (lz) continue; } catch (_) {}
  ...
  try {
    ...
    if (res && res.ok) {
      ...
      const _rb = {
        ...
        aviso_presupuesto_listo: 'Hola, soy Joaco de Neon Infinito. Cambiamos de número de WhatsApp y este es el nuevo, tal como avisamos en nuestras redes oficiales. Ya tenemos tu presupuesto listo para pasártelo. Respondé este mensaje y lo retomamos por acá.',
...
//  · YA recibió el link y ahora manda OTRO mensaje (respondió a NUESTRA respuesta, no un
//    re-click del walink) → recién ahí lo REVELAMOS en la bandeja de Abril ('cursos'). Así
//    no ensuciamos la bandeja con los que solo agarran el link y no vuelven a escribir.
async function lanzamientoLandingOnInbound(env, phone, msgBody) {
  ...
  try {
    ...
    if (stage === 'link_enviado') {
      // Ya tiene el link. Un mensaje que NO es el walink = respondió a nuestra respuesta → revelar.
      if (!isWalink) {
        const nowIso = new Date().toISOString();
        try { await env.DB.prepare("UPDATE lanzamiento_landing SET stage = 'revealed', updated_at = ? WHERE phone = ? AND stage = 'link_enviado'").bind(nowIso, phone).run(); } catch (_) {}
    ...
    if (stage === 'revealed') return; // ya visible en la bandeja de Abril
    // Todavía no tiene el link: se lo mandamos si tocó el walink, o si es un lead ya
    // registrado (respuesta al opener del Camino 2).
    if (isWalink || row) await sendEventoGroupLink(env, phone);
  } catch (_) {}
}
...
async function processPresupuestoFollowups(env, opts = {}) {
  ...
  for (const p of byPhone.values()) {
    ...
    if (conv.some(m => m.direction === 'outbound' && m.status !== 'failed' && (now - new Date(m.ts).getTime()) < 12 * 60 * 60 * 1000)) continue;
