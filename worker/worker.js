...
const GEMINI_RENDER_PROMPT = [
  ...
  '',
  '◆◆◆ EL ACRÍLICO ES 100% TRANSPARENTE — REGLA CRÍTICA ◆◆◆',
  'A través de cualquier forma del diseño (silueta del cuerpo de un perro, círculo de un balón, interior de una letra O, parte interna de cualquier figura) SE TIENE QUE VER LA PARED de fondo. No hay nada detrás de la manguera de neón salvo aire y luego el acrílico transparente, y detrás del acrílico la pared.',
  '',
  'PROHIBIDO ABSOLUTO en el render:',
  '- Vinilo, calcomanías, pintura, impresión.',
  '- Paneles LED, pantallas, módulos digitales.',
  '- RELLENO NEGRO, color sólido, o cualquier material opaco como fondo de las figuras. Aunque la imagen de referencia muestre una silueta negra del perro, esa silueta NO se rellena — solo se dibuja el contorno con neón.',
  '- Vinilo o lámina detrás del neón para "darle cuerpo" a la figura.',
  '- Sombras pintadas adentro de las figuras.',
  '- Texturas, degradados pintados, doble outline.',
  '- Cualquier material que no sea neón 6mm o acrílico transparente.',
  '',
  'CASOS QUE EL MODELO SUELE EQUIVOCAR (no lo hagas):',
  '- Una imagen de referencia con un perro en silueta negra → NO renderices el cuerpo del perro en negro/marrón sólido. Renderizalo como CONTORNO de neón (outline del perfil del perro) y por dentro se ve la pared gris.',
  '- Una imagen con relleno de color en una figura → SOLO outline, interior transparente.',
  '- Una imagen con bandas de colores diferentes adentro de una forma → o un solo color (el dominante) en la línea de contorno, o partís el contorno en secciones, pero NUNCA pintás el interior.',
  '',
  'TEST MENTAL antes de generar: si pudieras pasar la mano por detrás del cartel, deberías ver tu mano a través de cada forma (sin que tape nada salvo las mangueras de neón propiamente dichas). Si en tu render se ve un fondo opaco dentro de las figuras, está mal.',
  '',
  '═══ VISTA / CÁMARA ═══',
  ...
  '',
  '═══ RESTRICCIONES FÍSICAS DEL NEÓN LED — CRITERIOS DE SÍNTESIS ═══',
  'El neón LED tiene limitaciones físicas reales. NO sos un ilustrador haciendo arte digital — sos un fabricante adaptando un diseño a una manguera continua de silicona.',
  '',
  '◆◆◆ REGLA #0 (la más importante, se aplica ANTES que cualquier otra) ◆◆◆',
  'PROHIBIDO ABSOLUTO dibujar puntos, motas, destellos, sparkles, brillos pequeños, "polvo de estrellas", "pixels", "dots", chispitas, salpicaduras, manchas pequeñas o cualquier elemento puntual desconectado.',
  'Esto incluye:',
  '   · Puntos de color alrededor de las letras o figuras (sea como sea el color).',
  '   · Estrellitas/sparkles tipo "✨" decorativas en el fondo o flotando alrededor.',
  '   · Pequeñas formas geométricas dispersas (circulitos, rombitos, cruces chiquitas) como decoración.',
  '   · Detalles "estéticos" tipo confeti o glitter.',
  'Si la imagen de referencia los tiene, IGNORALOS. No los copies al render.',
  'Si tu instinto generativo te dice que el render "queda más bonito" con esos detalles, NO los agregues — son físicamente imposibles de fabricar (un punto de neón sería menor a 1cm de tubo y no se puede cortar).',
  'El cartel terminado solo tiene LÍNEAS CONTINUAS de neón formando el contorno del diseño principal. Nada más.',
  '',
  'REGLAS ADICIONALES:',
  '',
  '1. CORTE MÍNIMO: la manguera solo se puede cortar cada 1 cm. No existen tramos menores a 1cm de largo en el render.',
  '',
  '2. TAMAÑO MÍNIMO DE ELEMENTO RECONOCIBLE: cualquier figura/decoración debe medir al menos 2-3 cm en el cartel terminado para ser fabricable. Calculá esto contra las medidas finales del cartel (ancho/alto en cm).',
  '',
  '3. ELEMENTOS A SIMPLIFICAR (no eliminar, pero reducir):',
  '   · Estrellas decorativas medianas/grandes → contorno de estrella simple, solo si son parte importante del diseño (no decoración suelta).',
  '   · Detalles internos de figuras (chocolate con cuadrículas, líneas dentro de un objeto) → contorno exterior solo.',
  '   · Múltiples elementos chiquitos similares → agruparlos en una forma más grande continua o eliminarlos.',
  '   · Letras con doble outline (contorno y relleno de otro color) → una sola línea de contorno.',
  '',
  '4. SOLO CONTORNOS: el neón solo representa el contorno (outline) de cada forma. El interior de letras y figuras queda transparente (el acrílico).',
  '',
  '5. FONDO DEL CARTEL: el acrílico es transparente. Detrás del cartel solo se ve la pared (gris/blanca). No agregues elementos decorativos en el fondo del cartel ni "atmósfera" tipo partículas de luz.',
  '',
  '═══ INTERPRETACIÓN DEL INPUT ═══',
  '- Si la imagen es un boceto vectorizado limpio: mantené el diseño fiel.',
  '- Si es una foto, captura de chat o referencia rough: interpretá el diseño deseado y representalo SOLO con líneas de neón 6mm sobre acrílico transparente — no copies texturas, rellenos o materiales de la referencia.',
  '',
  '═══ PRIORIDAD DE LAS NOTAS DEL USUARIO ═══',
  'Si en el contexto vienen NOTAS / INSTRUCCIONES ESPECÍFICAS, aplicalas al render (color dentro de la paleta, tipografía, agregados, estilo, etc.).',
  'EXCEPCIONES que NUNCA pueden ser sobreescritas por las notas:',
  '- Materiales: siempre neón 6mm + acrílico transparente.',
  '- Vista frontal recta, nunca perspectiva.',
  '- Paleta de colores físicamente posibles. Si las notas piden un color que no existe (ej: "marrón"), aplicar el mapeo de la sección PALETA y aclararlo solo en el render fiel a la regla.',
  '- Criterios de síntesis y restricciones físicas (corte mínimo 1cm, tamaño mínimo 2-3cm, eliminación de puntitos/sparkles, etc.).'
].join('\n');
